...
*/
    ...
    int resultMC = 0;
    int avgTime = 0;
    int avgTime_denominator = 0;
    int avgIteration = 0;
    int avgIteration_denominator = 0;
    int fail = 0;
    int MaxSteps = 1000;
    //for detail txt
    ...
    for( int i=0; i<TRIAL_MC; ++i){
        ...
        if( *itr < 0 ){
            ...
            avgIteration_denominator++;
            FileD_MC<<'\t'<<i+1<<'\t'<<resultMC<<'\t'<<*itr<<'\t'<<MaxSteps<<'\t'<<TOTAL_STEPS_MC<<'\t'<<"SUCCESS"<<'\n';
        }
    }

    //for summary txt
    avgTime = avgTime/avgTime_denominator;
    avgIteration = avgIteration/avgIteration_denominator;
    ...
    FileD_MC<<"Avg:"<<'\t'<<"Q : "<<numOfQueens<<'\t'<<avgTime<<'\t'<<avgIteration<<'\t'<<MaxSteps<<'\t'<<TOTAL_STEPS_MC<<'\t'<<"SUCCESS"<<'\n';
