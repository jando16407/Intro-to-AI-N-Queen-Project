...
QueenGrid Min_Conflicts( struct QueenGrid QG, int MaxSteps ){
    ...
    for( int step=0; step<MAX_STEPS; ++step ){
        ...
        //std::cout<<"randomSelected queen pos =["<<selectedQueen.pos.x<<"]["<<selectedQueen.pos.y<<"]\n";
        ...
        //std::cout<<"NewPosition active="<<newPosition.active<<", numConflicts="<<newPosition.numOfConflicts<<" pos=["<<newPosition.pos.x<<"]["<<newPosition.pos.y<<"]\n";
        if( newPosition.numOfConflicts > 0 ){
            ...
            for( int k=0; k<numOfQueens; ++k ){
                ...
                */
            /*
                if( QG.ConflictQueens[k].active == false ){
                    int conf = calculateConflicts(QG.ConflictQueens[k], QG.Grid);
                    std::cout<<"CQ ["<<k<<"] conf ="<<conf<<std::endl;
                    if( conf > 0 ){
...
*/
        ...
        if( frequencyCount > numOfQueens*3 ){
            std::cout<<"UPdating avoidMap\n";
            for( int i=0; i<numOfQueens; ++i ){
                if( QG.ConflictQueens[i].active == true ){
                    std::cout<<"Active conflict at x = ["<<i<<"]\n";
                    if( QG.FrequencyMap[QG.ConflictQueens[i].pos.x][QG.ConflictQueens[i].pos.y] > numOfQueens && QG.AvoidMap[QG.ConflictQueens[i].pos.x][QG.ConflictQueens[i].pos.y] == false ){
                        ...
                        std::cout<<"New AvoidMap pos is ["<<QG.ConflictQueens[i].pos.x<<"]["<<QG.ConflictQueens[i].pos.y<<"]\n";
                    }
                }
            }
            frequencyCount = 0;
